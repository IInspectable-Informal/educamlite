...
extern "C" BOOL __stdcall ShowWindow(_In_ HWND hWnd, _In_ int nCmdShow);
extern "C" HWND __stdcall GetParent(_In_ HWND hWnd);

namespace winrt::educamlite::implementation
{
    StorageFolder TempFolder = nullptr;
    ...
    void MainPage::MinimizeRequested(IInspectable const&, RoutedEventArgs const&)
    ...
    fire_and_forget MainPage::TempFolderOperationRequested(IInspectable const& sender, RoutedEventArgs const&)
    {
        Button button = sender.as<Button>();
        button.IsEnabled(false);
        switch (button.Tag().as<hstring>()[0])
        {
            case L'0': //打开目录
            {
                _TempFolderManagerFlyout.Hide();
                if (!co_await Windows::System::Launcher::LaunchFolderAsync(TempFolder))
                { ShowInfo(muxc::InfoBarSeverity::Error, L"目录无法打开"); }
                break;
            }
            case L'1': //清空目录
            {
                Info().IsOpen(false); Info().Message(L"");
                auto subFiles = co_await TempFolder.GetFilesAsync();
                auto subDirs = co_await TempFolder.GetFoldersAsync();
                for (auto const& file : subFiles)
                {
                    try { co_await file.DeleteAsync(StorageDeleteOption::PermanentDelete); }
                    catch (hresult_error const& ex)
                    { Info().Message(Info().Message() + L"无法删除文件：" + file.Path() + L"，原因如下：\n" + ex.message() + L"\nHRESULT: " + ToHex(ex.code()) + L"\n"); }
                }
                for (auto const& dir : subDirs)
                {
                    try { co_await dir.DeleteAsync(StorageDeleteOption::PermanentDelete); }
                    catch (hresult_error const& ex)
                    { Info().Message(Info().Message() + L"无法删除文件夹：" + dir.Path() + L"，原因如下：\n" + ex.message() + L"\nHRESULT: " + ToHex(ex.code()) + L"\n"); }
                }
                _TempFolderManagerFlyout.Hide();
                if (Info().Message() == L"")
                { ShowInfo(muxc::InfoBarSeverity::Success, L"已成功清空目录"); }
                else
                {
            ...
            case L'2': //更改目录
            {
                _DirPicker.SettingsIdentifier(L"NewTempDirPicker");
                StorageFolder newTempFolder = co_await _DirPicker.PickSingleFolderAsync();
                if (newTempFolder)
                {
                    TempFolder = newTempFolder; newTempFolder = nullptr; FA.Clear();
                    AppDC.Values().Insert(L"FolderToken", box_value(FA.Add(TempFolder)));
                    FolderPathPresenter().Text(TempFolder.Path());
                } break;
            }
    ...
    fire_and_forget MainPage::CaptureRequested(IInspectable const& sender, RoutedEventArgs const&)
    {
        ...
        try
        {
            ...
            CaptureButton().IsEnabled(true);
            memStream.Seek(0);
            auto fileStream = co_await(co_await TempFolder.CreateFileAsync(GetCurrentTimeString() + SubtypeToFileExt(_Resolutions[i].Subtype()),
    ...
    void MainPage::Init(IInspectable const&, RoutedEventArgs const&)
    {
        ...
        {
            TempFolder = AppData.LocalFolder();
            propSet.Insert(L"FolderToken", box_value(FA.Add(TempFolder)));
        } else { TempFolder = FA.GetFolderAsync(unbox_value<hstring>(propSet.Lookup(L"FolderToken"))).get(); }
    ...
    MainPage::MainPage()
    {
        ...
        HWND childWindow;
        GetWindowHandle(CoreWindow::GetForCurrentThread(), &childWindow);
        mainWindow = GetParent(childWindow);
        if (mainWindow == NULL || mainWindow == nullptr)
        { mainWindow = childWindow; }
        _DirPicker.SuggestedStartLocation(PickerLocationId::ComputerFolder);
        _DirPicker.FileTypeFilter().Append(L"*");
        _FilePicker.SuggestedStartLocation(PickerLocationId::ComputerFolder);
        _FilePicker.FileTypeFilter().ReplaceAll({ hstring(L".jpg"), L".jpeg", L".png", L".bmp", L".tiff", L".tif", L".jxr", L".wdp", L".hdp", L".ico" });
    }
